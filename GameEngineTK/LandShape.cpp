...
bool LandShape::IntersectSphere(const Sphere& sphere, Vector3* reject)
{
	...
	localsphere._center = Vector3::Transform(sphere._center, m_WorldLocal);
	// 半径をワールドをワールド座標系からモデル座標系に変換
	localsphere._radius = sphere._radius / scale;
	...
	for (int i = 0; i < nTri; i++)
	{
		...
		{// ヒットした
			hit = true;
			// 衝突点から球の中心へのベクトル
			Vector3 sub = localsphere._center - temp_inter;
	...
	if (hit)
	{
		...
		if (reject != nullptr)
		{
			...
			*reject = Vector3::TransformNormal(l_normal, localworld);
			...
			float reject_distance = sphere._radius + over_length + extra;
